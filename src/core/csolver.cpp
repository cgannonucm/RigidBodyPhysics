...
namespace physics{
     ...
     */
    EVector CSolver::solve(const EVector &q, const EVector &v,const EVector &m,
     ...
     * @return Eigen 
     */
    Mat CSolver::CSolver::get_M_inv(EVector mv){
        using namespace Eigen;
        int n = mv.size();
        int dim = UDim * n;
     ...
     * @returns {J,Jd}
     */
    std :: array<Mat,2> CSolver::get_JJd(EVector q, EVector v, std :: vector<Constraint *> constraints){
     ...
     */
    std :: vector<Mat> CSolver::init_Kmats(int mcount, int dim){
        USING_STANDARD_NAMESPACES;
        vector<Mat> K;
        K.resize(mcount);
     ...
     * @return std (see description)
     */
    std :: vector<autodiff :: var> CSolver::get_qvar(EVector &q){
        using namespace :: std;
        auto cols = q.size();
        vector<var> qvar;
        qvar.resize(cols);
     ...
     */
    std :: vector<autodiff :: var> CSolver::eval_constraints(std :: vector<Constraint *> &constraints, std :: vector<var> &qvar){
        USING_STANDARD_NAMESPACES;
        using namespace autodiff;
     ...
     */
    std :: array<autodiff :: var,3> CSolver::grad_jx(int j, autodiff :: var *f, std :: vector<var> &qvar){
        using namespace autodiff;
     ...
     * @return std array of components of gradient
     */
    std :: array<double,3> CSolver::grad_j(int j, autodiff :: var *f, std :: vector<var> &qvar){
        using namespace autodiff;
        //Can't loop here because of the way wrt() is written
        int xpos = Universe :: get_pos(j,0);
        int ypos = Universe :: get_pos(j,1);
     ...
     */
    void CSolver::add_grad_to_mat(int i, int n, std :: array<autodiff :: var,3> &grad, Mat *M){
        for (int k = 0; k < 3; k++){
     ...
     * @return Eigen Jd
     */
    Mat CSolver::get_Jd(std :: vector<Mat> &K, EVector &v){
        USING_STANDARD_NAMESPACES;
